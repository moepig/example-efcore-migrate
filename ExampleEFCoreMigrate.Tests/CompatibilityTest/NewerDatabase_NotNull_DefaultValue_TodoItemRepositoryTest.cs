...
[TestFixture]
public class NewerDatabase_NotNull_DefaultValue_TodoItemRepositoryTest
{
    ...
    public async Task OneTimeSetUp()
    {
        ...
        // DbContextの設定 (db)
        var migrateDbOptions = new DbContextOptionsBuilder<NotNullDefaultValueApplicationDbContext>()
            .UseMySql(_mysqlContainer.GetConnectionString(), new MySqlServerVersion(new Version(8, 0, 43)))
            .Options;
        var migrateDbContext = new NotNullDefaultValueApplicationDbContext(migrateDbOptions);
