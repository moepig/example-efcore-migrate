...
[TestFixture]
public class NewerApplication_Nullable_TodoItemRepositoryTest
{
    private MySqlContainer _mysqlContainer;
    private NullableApplicationDbContext _dbContext;
    private NullableTodoItemRepository _repository;
    ...
    public async Task OneTimeSetUp()
    {
        ...
        // DbContextの設定 (app)
        var appDbOptions = new DbContextOptionsBuilder<NullableApplicationDbContext>()
            .UseMySql(_mysqlContainer.GetConnectionString(), new MySqlServerVersion(new Version(8, 0, 43)))
            .Options;
        _dbContext = new NullableApplicationDbContext(appDbOptions);
        ...
        // リポジトリの初期化
        _repository = new NullableTodoItemRepository(_dbContext);
    }
    ...
    {
        var todo = new NullableTodoItem { Title = "テスト", IsCompleted = false, Timestamp = DateTime.UtcNow, NewColumn = 100 };
